...
int cli_altaCliente(Cliente* list, int len, int* id)
{
	...
	if(list != NULL && len > 0 && id != NULL)
	{
		if(indice >= 0)
		{
			if(!utn_getNombre(auxCliente.nombre,64,"INGRESE NOMBRE: ","ERROR\n",2) &&
			   !utn_getNombre(auxCliente.apellido,64,"INGRESE APELLIDO: ","ERROR\n",2) &&
			   !utn_getCuit(auxCliente.cuit,21,"INGRESE CUIT: ","ERROR\n",2))
			{
	...
	if(list != NULL && len > 0)
	{
		...
		if(!utn_getNumero(&idClienteAModificar,"ELIJA EL CLIENTE A MODIFICAR: ","ERROR, NO EXISTE EL CLIENTE\n",1,21,2))
		{
			...
			if(indice >= 0)
			{
				...
				{
				case 1:
					if(!utn_getNombre(bufferChar,CADENA_LEN,"INGRESE NUEVO NOMBRE: ","ERROR, NO ES UN NOMBRE VALIDO",2)&&
					   !utn_confirmacionAccionChar("SEGURO QUE QUIERE CAMBIAR EL NOMBRE? INGRESE S/N"))
					{
						strncpy(list[indice].nombre,bufferChar,CADENA_LEN);
						printf("NOMBRE MODIFICADO CON EXITO\n");
					}
					break;
				case 2:
					if(!utn_getNombre(bufferChar,CADENA_LEN,"INGRESE NUEVO APELLIDO: ","ERROR, NO ES UN APELLIDO VALIDO",2)&&
					   !utn_confirmacionAccionChar("SEGURO QUE QUIERE CAMBIAR EL APELLIDO? INGRESE S/N"))
					{
						strncpy(list[indice].apellido,bufferChar,CADENA_LEN);
						printf("APELLIDO MODIFICADO CON EXITO\n");
					}
					break;
				case 3:
					if(!utn_getCuit(bufferChar,CADENA_LEN,"INGRESE NUEVO CUIT: ","ERROR, NO ES UN CUIT VALIDO",2)&&
					   !utn_confirmacionAccionChar("SEGURO QUE QUIERE CAMBIAR EL CUIT? INGRESE S/N"))
					{
						strncpy(list[indice].cuit,bufferChar,CADENA_LEN);
...

{
	...
	if(array != NULL && limite > 0 && indice < limite && indice >= 0 && id != NULL)
	{
			...
			strncpy(bufferCliente.nombre,nombre,CADENA_LEN);
			strncpy(bufferCliente.apellido,apellido,CADENA_LEN);
			strncpy(bufferCliente.cuit,cuit,21);
...
int cli_ordenarPorNombreCuit(Cliente* list,int len)
{
	...
	if(list != NULL && len > 0)
	{
		do
		{
			...
			for(i=0;i<len-1;i++)
			{
				auxiliarCmp = strncmp(list[i].nombre,list[i+1].nombre,CADENA_LEN);
				if(auxiliarCmp > 0 || (auxiliarCmp == 0 &&  strncmp(list[i].cuit,list[i+1].cuit,CADENA_LEN) > 0 ))
				{
					flagSwap = 1;
					buffer = list[i];
					list[i] = list[i+1];
