...
int main(void) {
	...
	Cliente* listaPunterosClientes[CLI_LEN];
	...
	if(!cli_inicializarConPunteros(listaPunterosClientes,CLI_LEN)
		/*!cli_inicializar(listaClientes,CLI_LEN)&&
	   !pub_inicializar(listaPublicaciones,PUB_LEN)*/)
	{

		//MEMORIA DINAMICA
		cli_altaForzadaArrayConPunteros(listaPunterosClientes,CLI_LEN,&idInicialCliente,"JOSE","NOGUEIRA","20-355422-1");
		cli_altaForzadaArrayConPunteros(listaPunterosClientes,CLI_LEN,&idInicialCliente,"NICOLAS","GOMEZ","21-352222-2");
		cli_altaForzadaArrayConPunteros(listaPunterosClientes,CLI_LEN,&idInicialCliente,"MATIAS","GIL","22-355422-3");
		cli_altaForzadaArrayConPunteros(listaPunterosClientes,CLI_LEN,&idInicialCliente,"ALEJANDRO","AVILA","23-355422-4");
		cli_altaForzadaArrayConPunteros(listaPunterosClientes,CLI_LEN,&idInicialCliente,"CRISTIAN","PEREZ","24-355422-5");
		...
		*/
		...
		do
		{
			...
			case 1:
				...
				cli_altaClienteConPunteros(listaPunterosClientes,CLI_LEN,&idInicialCliente);

			    break;
			case 2:
				if(cli_buscarClientesCargadosConPunteros(listaPunterosClientes,CLI_LEN) > 0)
				{
					cli_modificarClienteConPunteros(listaPunterosClientes,CLI_LEN);
				}
				else
				{
			...
			case 7:
				...
				cli_printClientesConPunteros(listaPunterosClientes,CLI_LEN);
				/*
				if(cli_buscarClientesCargados(listaClientes,CLI_LEN) >0)
				{
