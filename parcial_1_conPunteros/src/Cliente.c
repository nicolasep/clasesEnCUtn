...
int cli_inicializarConPunteros(Cliente** list, int len)
{
	int retorno = -1;
	int i;
	if(list != NULL && len > 0)
...
int cli_buscarLibreConPunteros(Cliente** list, int len)
{
	int retorno = -1;
	int i;
	if(list != NULL && len > 0)
...
int cli_printClienteConPunteros(Cliente* pCliente)
{
	int retorno = -1;
	if(pCliente != NULL)
	{
...
int cli_printClientesConPunteros(Cliente** list, int len)
{
	int retorno = -1;
	int i;
	...
	if(list != NULL && len > 0)
	{
		for(i=0; i<len; i++)
		{
			if(*(list+i) != NULL)
			{
				cli_printClienteConPunteros(*(&list[i]));
				retorno = 0;
			}
		}
...
int cli_buscarClientePorIdConPunteros(Cliente** list, int len, int id)
{
	int retorno = -1;
	int i;
...
int cli_altaClienteConPunteros(Cliente** list, int len, int* id)
{
	int retorno = -1;
	Cliente* auxCliente = cli_new();

	int indice = cli_buscarLibreConPunteros(list, len);

	if(list != NULL && len > 0 && id != NULL && auxCliente != NULL)
	{
		if(indice >= 0)
		{
			   ...
			   !utn_getCuit(auxCliente->cuit,CUIT_LEN,"INGRESE CUIT: ","ERROR\n",2))
			{
				...
				*id += 1;
				printf("ALTA EXITOSA\n");
				retorno = 0;
				cli_printClienteConPunteros(*(&list[indice]));
			}
			else
			{
...
int cli_modificarClienteConPunteros(Cliente** list, int len)
{
	int retorno = -1;
	...
	if(list != NULL && len > 0)
	{
		cli_printClientesConPunteros(list,len);
		if(!utn_getNumero(&idClienteAModificar,"ELIJA EL CLIENTE A MODIFICAR: ","ERROR, NO EXISTE EL CLIENTE\n",1,21,2))
		{
			indice = cli_buscarClientePorIdConPunteros(list,len,idClienteAModificar);
			if(indice >= 0)
			{
...
int cli_printClienteByIdConPunteros(Cliente** list, int len,int id)
{
	int retorno = -1;
	int i;
...
int cli_altaForzadaArrayConPunteros(Cliente** array,int limite, int* id,char* nombre,char* apellido, char* cuit)

{
	int retorno = -1;
	int indice = cli_buscarLibreConPunteros(array,limite);
	Cliente* bufferCliente = cli_new();

	if(bufferCliente != NULL && array != NULL && limite > 0 && indice < limite && indice >= 0 && id != NULL)
	{
...
int cli_buscarClientesCargadosConPunteros(Cliente** list, int len)
{
	int retorno = -1;
	int i;
 ...
 * */
int cli_inicializar(Cliente* list, int len)//listo
{
	int retorno = -1;
	int i;
	if(list != NULL && len > 0)
 ...
 * */
int cli_buscarLibre(Cliente* list, int len)//listo
{
	int retorno = -1;
	int i;
	if(list != NULL && len > 0)
 ...
 * */
int cli_printCliente(Cliente* pCliente)//listo
{
	int retorno = -1;
	if(pCliente != NULL)
	{
 ...
 * */
int cli_printClientes(Cliente* list, int len)//listo
{
	int retorno = -1;
	int i;
